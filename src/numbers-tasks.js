...
function getAverage(value1, value2) {
  ...
  if (maxValue < 0) {
    return minValue + Math.abs(maxValue - minValue) / 2;
  }
...
function toNumber(value, def) {
  const result = Number.isFinite(+value) ? +value : def;
  return result;
}
...
function getCountOfOddNumbers(number) {
  let counter = 0;
  let countNumber = number < 0 ? -number : number;
  for (countNumber; countNumber > 0; countNumber -= 2) {
    counter += 1;
  }
  return counter;
}
